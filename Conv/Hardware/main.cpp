...
/*
int16_t ifm[1024] = { 0 };
int16_t weight_rg[1024000] = { 0 };
int16_t ofm[1024000] = { 0 };
int16_t bias[1024] = { 0 };
int16_t ofm_soft[1024000] = { 0 };
int16_t weight[1024000] = { 0 };
*/
void weight_rearrangement(trans_type* weight, trans_type* weight_rg, int ofm_num, int ifm_num, int ksize)
{
	int w_cnt = 0;
...
void conv_soft(int ifm_num, int ifm_h, int ifm_w, int ofm_num, int ofm_h, int ofm_w, int ksize, int stride,
	trans_type* ifm, int inputQ, trans_type* weight, int weightQ, trans_type* ofm, int outputQ,
	trans_type* bias, int biasQ, int pad_int)
{
				...
				for (int c = 0; c < ifm_num; c++)
					for(int ii=0;ii<ksize;ii++)
						for (int jj = 0; jj < ksize; jj++)
						{
							...
							{
								trans_type dat = ifm[c * ifm_h * ifm_w + row * ifm_w + col];
								trans_type wt = weight[i * ifm_num * ksize * ksize + c * ksize * ksize + ii * ksize + jj];
								sum += dat * wt;
							}
						}
				int res = (sum >> (inputQ + weightQ - outputQ)) + (bias[i] >> (biasQ - inputQ));
...
int main()
{
	trans_type ifm[3000] = { 0 };
	trans_type weight_rg[2000] = { 0 };
	trans_type ofm[60000] = { 0 };
	trans_type bias[1024] = { 0 };
	trans_type ofm_soft[60000] = { 0 };
	trans_type weight[2000] = { 0 };
	int ksize = 3;
	int stride = 1;
	int pad_int = 1;
	int ifm_h = 27;
		...
		for(int i=0;i<ofm_h;i++)
			for (int j = 0; j < ofm_w; j++)
			{
				if (ofm[c * ofm_h * ofm_w + i * ofm_w + j] != ofm_soft[c * ofm_h * ofm_w + i * ofm_w + j])
				{
					...
					printf("Out_Soft[%d][%d][%d]=%d\n", c, i, j, ofm_soft[c * ofm_h * ofm_w + i * ofm_w + j]);
					*/
					cout<<"Out_"<<c<<i<<j<<" "<<ofm[c * ofm_h * ofm_w + i * ofm_w + j]<<endl;
					cout<<"Out_"<<c<<i<<j<<" "<<ofm_soft[c * ofm_h * ofm_w + i * ofm_w + j]<<endl;
				}
			}
	if (flag == 1)
		printf("============================\n   result_match\n============================\n");
