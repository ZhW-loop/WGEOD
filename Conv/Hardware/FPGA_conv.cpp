...
void ifm_mmcpy_row(trans_type *input, trans_type local_buf[OnChipIB_Width+3], int CurrentOffset, int Roffset,uint32_t IHxIW, int IW_align_256b, uint16_t TCol,
...
//#pragma HLS INTERFACE m_axi depth=512 port=input offset=slave max_read_burst_length=128 max_write_burst_length=128
	...
	memcpy(local_buf, (trans_type *)(input + ifm_offset), TCol*sizeof(trans_type));
	*t1_n = t1;
	*t2_n = t2;
}

void ifm_copy_lbuf2ibuf(trans_type input_buffer[Tn][OnChipIB_Height][OnChipIB_Width], trans_type local_buf[OnChipIB_Width+3], uint16_t TCol, uint16_t Input_w, uint16_t Input_h, uint8_t TN_MIN,
...
void input_load(trans_type *input, trans_type input_buffer[Tn][OnChipIB_Height][OnChipIB_Width], uint16_t r, uint16_t c, uint16_t n, uint8_t Kstride, uint8_t Padding,
...
//#pragma HLS INTERFACE m_axi depth=512 port=input offset=slave max_read_burst_length=128 max_write_burst_length=128
	...
	uint8_t t1,t2;
	static trans_type local_buf0[OnChipIB_Width+3];
	static trans_type local_buf1[OnChipIB_Width+3];
...
void weight_load_reorg(trans_type *Weight,trans_type weight_buffer[Tm][Tn][K][K],
                       int m,int n, uint8_t Ksize, uint8_t TM_MIN, uint8_t TN_MIN, bool enable, uint16_t TN, uint16_t TM)
{
	...
	uint8_t t1,t2,t3,t4;
	static trans_type local_buf[Tm*Tn*K*K + 3];
	static int Woffset;
	...
	memcpy(local_buf, (trans_type *)(Weight + Woffset), mm_offset*sizeof(trans_type));
	//Woffset += TN * TM * KxK;
	Woffset += mm_offset;
	uint16_t cnt = 0;
...
void nonlinear_leaky_row(trans_type local_buf[Tc], ap_int<36> Input[Tm][Tr][Tc], trans_type local_beta_buffer[Tm], uint8_t tm, uint8_t tr, uint8_t *tm_n, uint8_t *tr_n, uint8_t TC_MIN,
...
//	float tmp_out, tmp, tmp1;
	...
	ap_int<36> tmp_out, tmp1, tmp0;

	trans_type tmp_int16;
...
#pragma HLS PIPELINE II=1
		...
		{
			if(tmp1 < 0)
				tmp_out = ((ap_int<36>)tmp1*0xccc)>>15;
			else
				tmp_out = tmp1;
		}
		else
...
void ofm_mmcpy_row(trans_type *Output, trans_type local_buf[Tc], int offset, uint32_t OHxOW, uint16_t Output_w, uint8_t TC_MIN, uint8_t tm, uint8_t tr,bool enable)
{
	if(!enable)
		return;
	...
	memcpy((trans_type *)(Output + trans_offset), local_buf, loop_cnts*sizeof(trans_type));

}

void copy_local_beta(trans_type beta_buffer[MAX_BETA_LENGTH], trans_type local_beta_buffer[Tm], const int TM_MIN, int m, int InterSubBeta)
{
...
#pragma HLS PIPELINE II=1
		local_beta_buffer[tm] = (trans_type)beta_buffer[offset] << InterSubBeta;
		offset++;
	}
}

void write_back_output_reorg(ap_int<36> output_buffer[Tm][Tr][Tc], trans_type *Output, trans_type beta_buffer[MAX_BETA_LENGTH], uint16_t r,uint16_t c,uint16_t m, uint16_t Output_w,
	...
	int InterSubBeta, int WeightAddInputSubInter)
{
	if(!enable)
		return;
	static trans_type local_beta_buffer[Tm];
	...
	static trans_type local_buf0[Tc];
	static trans_type local_buf1[Tc];
...
void conv2d_tile(trans_type input_buffer[Tn][OnChipIB_Height][OnChipIB_Width],ap_int<36> output_buffer[Tm][Tr][Tc],
		trans_type weight_buffer[Tm][Tn][K][K],int n_next,
...
#pragma HLS DEPENDENCE variable=output_buffer inter false
						if(i==0&&j==0&&ne0)
							...
							partial_add[tmm] = output_buffer[tmm][trr][tcc];

						for(tnn = 0;tnn <Tn;tnn++)
						{
#pragma HLS UNROLL
							partial_mul[tmm][tnn] = weight_buffer[tmm][tnn][i][j] * input_buffer[tnn][Kstride_3b*trr+i][Kstride_3b*tcc+j];
						}

						ap_int<36> partial_sum = 0;
						for(tnn = 0;tnn <Tn;tnn++)
						{
#pragma HLS UNROLL
							 partial_sum += partial_mul[tmm][tnn];
						}
						output_buffer[tmm][trr][tcc] = (partial_add[tmm] + partial_sum);
						//printf("%d\n", output_buffer[tm][tr][tc]);
					}
				}
}

void ifm_weight_load_wrapper(trans_type *ifm, trans_type *weight, trans_type ifm_buffer[Tn][OnChipIB_Height][OnChipIB_Width], trans_type weight_buffer[Tm][Tn][K][K],
...
{
//#pragma HLS INTERFACE m_axi depth=512 port=ifm offset=slave max_read_burst_length=128 max_write_burst_length=128
//#pragma HLS INTERFACE m_axi depth=512 port=weight offset=slave max_read_burst_length=128 max_write_burst_length=128
	if(!enable)
		return;
...
void load_compute_wrapper(trans_type *ifm, trans_type *weight, ap_int<36> ofm_buffer[Tm][Tr][Tc], trans_type bias_buffer[MAX_BETA_LENGTH], int ksize, uint8_t K_1, int kstride, int ifm_num, int ifm_w, int IW_align_256b,
...
{
	static trans_type ifm_buffer0[Tn][OnChipIB_Height][OnChipIB_Width];
#pragma HLS ARRAY_PARTITION variable=ifm_buffer0 complete dim=1
	static trans_type ifm_buffer1[Tn][OnChipIB_Height][OnChipIB_Width];
#pragma HLS ARRAY_PARTITION variable=ifm_buffer1 complete dim=1

	static trans_type weight_buffer0[Tm][Tn][K][K];
...
#pragma HLS ARRAY_PARTITION variable=weight_buffer0 complete dim=2
	static trans_type weight_buffer1[Tm][Tn][K][K];
...
void copy_beta(trans_type beta_buffer[MAX_BETA_LENGTH], trans_type *Beta, uint16_t OFM_NUM)
{
	static trans_type local_buf[MAX_BETA_LENGTH];
	uint16_t NUM = OFM_NUM;

	memcpy(local_buf, Beta, NUM*sizeof(trans_type));
...
#pragma HLS INTERFACE m_axi depth=1024 port=bias offset=slave bundle=BUS1 max_read_burst_length=128 max_write_burst_length=128
#pragma HLS INTERFACE m_axi depth=2000 port=weight offset=slave bundle=BUS1 max_read_burst_length=128 max_write_burst_length=128
#pragma HLS INTERFACE m_axi depth=3000 port=ifm offset=slave bundle=BUS0 max_read_burst_length=128 max_write_burst_length=128
#pragma HLS INTERFACE m_axi depth=60000 port=ofm offset=slave bundle=BUS0 max_read_burst_length=128 max_write_burst_length=128
